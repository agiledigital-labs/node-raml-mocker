...
exports.ramlmocker = async (port, ramlFile, transformers) => {
  ...
  var responseTransformers = transformers
  ...
  const get = id =>
    responseTransformers.find(transformer => transformer.id === id);
  ...
  app.use(
    mung.write((chunk, encoding, request, response) => {
      responseTransformers.forEach(element => {
        console.log(element);
      });

      const matchesPath = transformer =>
        !transformer.path || transformer.path === request.route.path;

      console.log(request.route.path);

      const reducer = (body, transformer) =>
        transformer.transform(body, request, response) || body;
